...
import { BookingPolicy } from "../(frontend)/utils/validation/booking.policy";
import { EmailService } from "./email.service";
import { generateICS } from "./generateEmail";
import { query } from "../(frontend)/utils/neon";
import { BookingPayload } from "@/app/types/booking";
import { checkCalendarConflict, createBookingEvent, deleteCalendarEvent, getCalendarAuth } from "@/app/utils/google-calendar";
import { createZoomMeeting, deleteZoomMeeting } from "@/app/utils/zoom";
import { loadServerMessages } from "../../messages/server";
import { convertJSTToUserTimezone, hashCode, weeklySlots } from "@/app/utils/slots";
...
export const BookingService = {
	...
	async rescheduleBooking(token: string, date: string, time: string, locale: string = "ja") {
		...
		try {
			...
			// Create New
			const zoomTopic = `(JBS) Free Coaching X ${oldBooking.first_name} ${oldBooking.last_name}`;
			...
			const messages = await loadServerMessages(locale);
			const managementUrl = `${process.env.NEXT_PUBLIC_APP_URL}/${locale}/free-coaching/manage/${newBooking.cancellation_token}`;
