...
export default function SingleRowTestimonials({ data }: TestimonialsProps) {
	// If the block is missing from the layout, return null to avoid layout shift
	if (!data || !data.items) {
		return null;
	}
	...
	// Custom parser options for the <accent> tag defined in Payload
	...
	return (
		<section className="bg-white py-12 px-6 scroll-mt-16" id="testimonials">
			<div className="max-w-7xl mx-auto">
				...
				<div className="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-4">
					<div className="max-w-xl">
						<div className="flex items-center gap-2 mb-2">
							...
							<span className="text-[10px] font-black tracking-[0.25em] uppercase text-gray-400">{badge || "Testimonials"}</span>
						</div>
				...
				<div className="grid grid-cols-1 md:grid-cols-3 gap-4 lg:gap-6">
					{items?.map((item: any, idx: number) => (
						<div key={item.id || idx} className="group flex flex-col justify-between p-6 rounded-2xl border border-gray-100 bg-[#fafafa] hover:bg-white hover:shadow-xl hover:shadow-blue-900/5 transition-all duration-300">
							<div>
								{/* Stars & Indicator */}
