							...
							<div className="md:col-span-8 bg-white p-5 md:p-10 flex flex-col justify-between">
								<div>
									...
									<form className="grid gap-6" onSubmit={(e) => e.preventDefault()}>
										<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
											{nameFieldConfig.map((item) => (
												<Input
													key={item.field}
													label={item.label}
													name={item.field} // e.g. "firstName"
													// Map your field names to standard browser tokens
													autoComplete={item.field === "firstName" ? "given-name" : "family-name"}
										...
										<Input label={t("booking.email")} type="email" name="email" autoComplete="email" value={formData.email} onChange={(val) => updateField("email", val)} required />
										<div className="flex flex-col gap-2">
											<label className="text-xs font-bold uppercase tracking-widest text-gray-400">{t("booking.message")}</label>
											<textarea rows={4} name="message" value={formData.message} onChange={(e) => updateField("message", e.target.value)} className="rounded-xl border border-gray-200 p-4 text-sm focus:outline-none focus:ring-2 focus:ring-[#1754cf]/30" />
										</div>{" "}
									</form>
								</div>
...
function Input({ label, type = "text", value, onChange, required = false, name, autoComplete }: { label: string; type?: string; value: string; onChange: (val: string) => void; required?: boolean; name?: string; autoComplete?: string }) {
	const inputId = label.toLowerCase().replace(/\s+/g, "-");

	return (
		<div className="flex flex-col gap-2">
			<label htmlFor={inputId} className="text-xs font-bold uppercase tracking-widest text-gray-400">
				{label}
				{required && <span className="ml-1 text-red-500">*</span>}
			</label>
			<input id={inputId} name={name} autoComplete={autoComplete} type={type} value={value} onChange={(e) => onChange(e.target.value)} required={required} className="rounded-xl border border-gray-200 p-4 text-sm focus:outline-none focus:ring-2 focus:ring-[#1754cf]/30" />
		</div>
	);
}
