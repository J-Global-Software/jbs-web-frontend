...
export default function WorkshopDetail({ workshop, code, levelLabel = true, showSubtitle = true }: WorkshopDetailProps) {
	...
	const speakersList = cleanSpeakers(workshop.speakers || []);
	const objectivesList = parseList(workshop.objectives);
	const purposeList = parseList(workshop.purpose);
	const participantsList = parseList(workshop.participants);
	const languageList = parseList(workshop.language);
