...
export default function WorkshopDetail({ workshop, code, levelLabel = true, showSubtitle = true, isBiznite = false }: WorkshopDetailProps) {
    ...
    // Safety check: Map sessions to ensure all required fields exist before passing to children
    const safeSessions = useMemo(() => {
        return (workshop.sessions || []).map(session => ({
            ...session,
            dates: (session.dates || []).map(d => ({
                ...d,
                endTime: d.endTime || "",
                zoomLink: d.zoomLink || "",
                eventId: d.eventId || ""
    ...
    return (
        <main className="min-h-screen bg-[#F8FAFC] selection:bg-blue-100">
            ...
            <WorkshopHero 
                ...
                subtitle={showSubtitle ? workshop.subtitle : ""} 
                image={workshop.image} 
                code={code} 
                levelLabel={levelLabel ? tLevels(code) : ""} 
                registerNowLabel={tPrograms("registerNow")} 
                readMoreLabel={tPrograms("readMore")} 
                readLessLabel={tPrograms("readLess")} 
                locale={locale} 
            />
