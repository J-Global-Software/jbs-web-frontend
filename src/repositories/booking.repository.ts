import { Booking, BOOKING_STATUS, BookingStatus, CreateBookingDTO } from "@/types/booking";
...
export const BookingRepository = {
	...
	async updateStatus(id: string, status: BookingStatus) {
		...
		if (timestampCol) {
			...
			return await query(`UPDATE jbs.bookings SET status = $1, ${timestampCol} = $2 WHERE id = $3`, [status, now, id]);
		}

		return await query(`UPDATE jbs.bookings SET status = $1 WHERE id = $2`, [status, id]);
	},

	async findByToken(token: string): Promise<Booking | null> {
		const res = await query<Booking>(`SELECT * FROM jbs.bookings WHERE cancellation_token = $1`, [token]);
		return res.rows[0] || null;
	},

	async findLatestChild(originalId: string): Promise<Booking | null> {
		const res = await query<Booking>(
			`SELECT * FROM jbs.bookings 
